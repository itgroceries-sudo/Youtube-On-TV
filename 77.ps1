...
# =========================================================
#  YOUTUBE TV INSTALLER v77.0 (SEAMLESS UI)
#  Status: NO Close Button (Gone) | Layout Synced
# =========================================================
...
$SelfURL = "$GitHubRaw/77.ps1"
$AppVersion = "2.0 Build 24.77"
$BuildDate  = "30-1-2026"
...
# --- [2. WIN32 API] ---
# Added Get/SetWindowLong to REMOVE the System Menu (The whole X button area)
$User32Def = @'
...
if ($Silent) {
    ...
    # [REQ 1] COMPLETELY REMOVE 'X' BUTTON (Remove WS_SYSMENU)
    $GWL_STYLE = -16
    $WS_SYSMENU = 0x00080000
    $CurrentStyle = [Win32.User32]::GetWindowLong($ConsoleHandle, $GWL_STYLE)
    if ($CurrentStyle -ne 0) {
        # Bitwise AND with NOT SYSMENU to strip it out
...
if(!$Silent){ 
    ...
    Write-Host "   (V.2 Build 24.77 : $BuildDate)         " -ForegroundColor Green
...
# [REQ 1] Remove GUI Close Button (Using Get/SetWindowLong)
$Window.Add_SourceInitialized({
    $hwnd = (New-Object System.Windows.Interop.WindowInteropHelper($Window)).Handle
    $GWL_STYLE = -16
    $WS_SYSMENU = 0x00080000
    $CurrentStyle = [Win32.User32]::GetWindowLong($hwnd, $GWL_STYLE)
    [Win32.User32]::SetWindowLong($hwnd, $GWL_STYLE, ($CurrentStyle -band (-not $WS_SYSMENU))) | Out-Null
})

# [REQ 2] Magnet & Layout Fix: Force Console HEIGHT to match WPF HEIGHT
$Window.Add_LocationChanged({
    if (!$Silent) {
        $ConsX = ($Window.Left * $Scale) - $ConsoleW_Px - ($Gap)
        $ConsY = ($Window.Top * $Scale)
        # [FIX] Use $Window.ActualHeight * $Scale to match Console Height exactly to Menu Height
