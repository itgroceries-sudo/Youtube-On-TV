...
# =========================================================
#  YOUTUBE TV INSTALLER v75.6.7 (SILENT OPTION)
#  Version: 2.0 Build 23.75.6.7
#  File: 7567.ps1 | Branch: branch
#  Status: Added -StartMenu argument for Silent Mode
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.6.7"
...
for ($i = 0; $i -lt $AllArgs.Count; $i++) {
    ...
    # [UPDATE] Check for -StartMenu flag
...
if (-not $Silent -and -not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    # Pass -StartMenu to admin process if present
    ...
    } catch { Write-Host "`n [ERROR] $_" -ForegroundColor Red; Read-Host }
    exit 
}
...
$BaseW = 580; $BaseH = 880; $ConsoleW_Px = [int]($BaseW * $Scale); $ConsoleH_Px = [int]($BaseH * $Scale)
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.6.7 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
...
# [UPDATE] Silent Mode Handler now accepts $AddStartMenu
if ($Silent -or ($Browser -ne "Ask")) { 
    foreach($b in $Global:Browsers){ 
        if($b.N -match $Browser -or $b.K -match $Browser){ 
            ...
            if($FP){ 
                $b.Path=$FP
                # Install with Desktop=True (Default) and StartMenu=$AddStartMenu
                Install-Browser $b.N $false $AddStartMenu 
            } 
        } 
    }; 
    exit 
...
Title="YouTube TV Installer" Height="$BaseH" Width="$BaseW" WindowStartupLocation="Manual" ResizeMode="NoResize" Background="#181818" Topmost="True" WindowStyle="None" BorderBrush="#2196F3" BorderThickness="4">
    <Window.Resources>
        ...
        <Style x:Key="VectorBtn" TargetType="Button"><Setter Property="Background" Value="#333333"/><Setter Property="BorderThickness" Value="0"/><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="45"/><Setter Property="Height" Value="45"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border x:Name="b" Background="{TemplateBinding Background}" CornerRadius="22"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter TargetName="b" Property="Opacity" Value="0.8"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
        <Style x:Key="HeaderBtn" TargetType="Button"><Setter Property="Background" Value="Transparent"/><Setter Property="BorderThickness" Value="0"/><Setter Property="Cursor" Value="Hand"/><Setter Property="Width" Value="30"/><Setter Property="Height" Value="30"/><Setter Property="Template"><Setter.Value><ControlTemplate TargetType="Button"><Border Background="{TemplateBinding Background}"><ContentPresenter HorizontalAlignment="Center" VerticalAlignment="Center"/></Border><ControlTemplate.Triggers><Trigger Property="IsMouseOver" Value="True"><Setter Property="Opacity" Value="0.7"/></Trigger></ControlTemplate.Triggers></ControlTemplate></Setter.Value></Setter></Style>
    </Window.Resources>
    ...
    <Grid Margin="25">
        ...
        <Grid Grid.Row="4">
            ...
            <StackPanel Orientation="Horizontal" Grid.Column="0">
                <Button x:Name="BDesk" Style="{StaticResource VectorBtn}" Margin="0,0,8,0" ToolTip="Add to Desktop (Default)" Cursor="Hand" Tag="On" Background="#0078D7"><Viewbox Width="24" Height="24"><Path Fill="White" Data="M21 2H3c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h7v2H8v2h8v-2h-2v-2h7c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H3V4h18v12z"/></Viewbox></Button>
...
$BWin = $Window.FindName("BWin"); $BTrash = $Window.FindName("BTrash"); $BDesk = $Window.FindName("BDesk")
...
$BA.Add_Click({ Play-Sound "Click"; $Sel = $Stack.Children | Where-Object { $_.Child.Children[2].IsChecked }; if ($Sel.Count -eq 0) { return }; $BA.IsEnabled = $false; $BA.Content = "Processing..."; $IsUninstall = ($BTrash.Tag -eq "On"); $UseStartMenu = ($BWin.Tag -eq "On"); $UseDesktop = ($BDesk.Tag -eq "On"); foreach ($i in $Sel) { Install-Browser $i.Child.Children[2].Tag $IsUninstall $UseStartMenu $UseDesktop }; $BA.Content = "Finished"; Play-Sound "Done"; Start-Sleep 2; $BA.IsEnabled = $true; if ($IsUninstall) { $BA.Content = "Uninstall Selected" } else { $BA.Content = "Start Install" } })
