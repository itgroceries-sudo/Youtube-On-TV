...
# =========================================================
#  YOUTUBE TV INSTALLER v75.2 (INTERNAL CONSOLE)
#  Based on: v75.1 (Stable Base)
#  Changes: Internal LogBox | Blue Border | Ghost Console
# =========================================================

# --- [1. WIN32 GHOSTING (HIDE REAL CONSOLE)] ---
$User32Def = @'
[DllImport("kernel32.dll")] public static extern IntPtr GetConsoleWindow();
[DllImport("user32.dll")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);
'@
$Win32 = Add-Type -MemberDefinition $User32Def -Name "Win32Ghost" -Namespace Win32 -PassThru
[Win32.Win32Ghost]::ShowWindow([Win32.Win32Ghost]::GetConsoleWindow(), 0) | Out-Null # Hide Console Completely
...
if (-not $Silent -and -not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    # Request Admin (Console is hidden, so we just relaunch)
    $PassArgs = @()
...
# Helper to log to GUI instead of Console
...
# GUI XAML (Modified for Internal Console & Blue Border)
[xml]$xaml = @"
...
$Window.Add_MouseLeftButtonDown({ $this.DragMove() }) # Enable dragging for no-titlebar window
...
Write-Log "------------------------------------------"
Write-Log "   (V.2 Build 23.75.2 : $BuildDate)"
Write-Log "------------------------------------------"
...
function Install-Browser {
    ...
    Write-Log "[INSTALL] $($Obj.N)... DONE" "Green"
}
...
foreach ($b in $Global:Browsers) {
    ...
    $Bor.Add_MouseLeftButtonUp({
        ...
        else { if ($sender.Tag) { Start-Process $sender.Tag } }
    })
}
