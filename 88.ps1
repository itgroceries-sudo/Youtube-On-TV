...
# =========================================================
#  YOUTUBE TV INSTALLER v88.0 (THE INVISIBLE CORE)
#  Status: ZERO External Window | Pure GUI Experience
# =========================================================

# --- [1. WIN32 GHOSTING] ---
# รีบสั่งซ่อนหน้าต่างตัวเองตั้งแต่บรรทัดแรกสุด เพื่อไม่ให้หน้าต่างดำโผล่มา
$Win32Code = @'
[DllImport("kernel32.dll")] public static extern IntPtr GetConsoleWindow();
[DllImport("user32.dll")] public static extern bool ShowWindow(IntPtr hWnd, int nCmdShow);
'@
$Win32 = Add-Type -MemberDefinition $Win32Code -Name "Win32Ghost" -Namespace Win32 -PassThru
[Win32.Win32Ghost]::ShowWindow([Win32.Win32Ghost]::GetConsoleWindow(), 0) | Out-Null # 0 = SW_HIDE
...
$SelfURL = "$GitHubRaw/88.ps1"
$AppVersion = "2.0 Build 32.88"
...
if (-not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    if (!$IsLocal) {
        ...
        Start-Process PowerShell -ArgumentList @("-NoProfile", "-ExecutionPolicy", "Bypass", "-WindowStyle", "Hidden", "-File", "`"$TempScript`"") -Verb RunAs
    } else {
        Start-Process PowerShell -ArgumentList @("-NoProfile", "-ExecutionPolicy", "Bypass", "-WindowStyle", "Hidden", "-File", "`"$TargetFile`"") -Verb RunAs
    }
    exit 
}

# --- [3. PREPARE GUI & ASSETS] ---
Add-Type -AssemblyName PresentationFramework, System.Windows.Forms, System.Drawing
if (-not (Test-Path $InstallDir)) { New-Item -ItemType Directory -Force -Path $InstallDir | Out-Null }

$Assets = @{
    "MenuIcon" = "$GitHubRaw/YouTube.ico"; "Chrome"="$GitHubRaw/IconFiles/Chrome.ico"; "Edge"="$GitHubRaw/IconFiles/Edge.ico"
    "Brave"="$GitHubRaw/IconFiles/Brave.ico"; "Vivaldi"="$GitHubRaw/IconFiles/Vivaldi.ico"; "Yandex"="$GitHubRaw/IconFiles/Yandex.ico"
    "Chromium"="$GitHubRaw/IconFiles/Chromium.ico"; "Thorium"="$GitHubRaw/IconFiles/Thorium.ico"
}
...
Title="YouTube TV Installer" Height="880" Width="520" WindowStartupLocation="CenterScreen" ResizeMode="NoResize" Background="#181818" Topmost="True" WindowStyle="None" BorderBrush="#2196F3" BorderThickness="2">
    ...
    <Grid Margin="25,20,25,25">
        ...
        <Border Grid.Row="3" Background="#000000" CornerRadius="8" Margin="0,15,0,0" BorderBrush="#333333" BorderThickness="1"><ScrollViewer x:Name="LogScroll" VerticalScrollBarVisibility="Auto"><TextBlock x:Name="LogBox" Foreground="#00FF00" FontFamily="Consolas" FontSize="11" Margin="10" TextWrapping="Wrap" Text="[SYSTEM] Initializing Stealth Core..."/></ScrollViewer></Border>
...
$Window.FindName("BA").Add_Click({
    ...
    foreach ($i in $Sel) {
        ...
        $Obj = $Global:Browsers | Where-Object { $_.N -eq $BName }
        $FP=$null; foreach($p in $Obj.P){ if($p -and (Test-Path $p)){ $FP=$p; break } }
        if($FP){
            $Sut = Join-Path ([Environment]::GetFolderPath("Desktop")) "YouTube On TV - $($Obj.N).lnk"
            $Ws = New-Object -Com WScript.Shell; $s = $Ws.CreateShortcut($Sut); $s.TargetPath = "cmd.exe"
