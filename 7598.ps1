...
# =========================================================
#  YOUTUBE TV INSTALLER v75.9.8 (BUFFER FIX)
#  Version: 2.0 Build 23.75.9.8
#  File: 7598.ps1 | Branch: branch
#  Status: Fixed Buffer Size | Forced Icon Load | Fitted Launcher
# =========================================================

# [0] PRE-INIT: FORCE TLS 1.2 (Must be first)
[System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12

# ---------------------------------------------------------
# [1] CONFIG & LANGUAGE
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.9.8"
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    # Ensure Launcher Size
    try { $host.UI.RawUI.WindowSize = New-Object System.Management.Automation.Host.Size(80, 15) } catch {}
...
if ($Silent) { if ($ConsoleHandle) { [Win32.User32]::ShowWindow($ConsoleHandle, 0) | Out-Null } } else {
    # [FIX] EXPAND BUFFER FIRST to allow Window Resize
...
foreach($k in $Assets.Keys){ 
    $D="$InstallDir\$k.ico"
    # [FIX] Force redownload if file is empty (0kb)
    if(!(Test-Path $D) -or (Get-Item $D).Length -lt 100){ 
        try{ 
            ...
            if(!$Silent){ Write-Host " [ERROR]    Fail: $k" -ForegroundColor Red }
        } 
    } else { 
        if(!$Silent){ Write-Host " [CACHE]    OK: $k" -ForegroundColor DarkGray } 
    } 
}

# Reload Icon again in case it was just downloaded
$ConsoleIcon = "$InstallDir\ConsoleIcon.ico"
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.9.8 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
