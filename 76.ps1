...
# =========================================================
#  YOUTUBE TV INSTALLER v76.0 (UNIFIED UI)
#  Status: No-Close(X) | Magnet Windows | TopMost Console
# =========================================================
...
$SelfURL = "$GitHubRaw/76.ps1"
$AppVersion = "2.0 Build 24.76"
$BuildDate  = "30-1-2026"
...
# --- [2. WIN32 API & CONSOLE PREP] ---
# [REQ 1 & 2] Added DeleteMenu, GetSystemMenu for disabling 'X' and TopMost logic
$User32Def = @'
...
if ($Silent) {
    ...
    # [REQ 2] TopMost Console (-1) | [REQ 1] Disable Close Button Logic later
    ...
    # [REQ 1] Disable Console Close Button (The 'X')
    $SC_CLOSE = 0xF060
    $MF_BYCOMMAND = 0x00000000
    $hMenu = [Win32.User32]::GetSystemMenu($ConsoleHandle, $false)
    if ($hMenu) { [Win32.User32]::DeleteMenu($hMenu, $SC_CLOSE, $MF_BYCOMMAND) | Out-Null }
...
if(!$Silent){ 
    ...
    Write-Host "   (V.2 Build 24.76 : $BuildDate)         " -ForegroundColor Green
...
# [REQ 1] Disable GUI Close Button logic (Hooking Window Handle)
$Window.Add_SourceInitialized({
    $hwnd = (New-Object System.Windows.Interop.WindowInteropHelper($Window)).Handle
    $hMenu = [Win32.User32]::GetSystemMenu($hwnd, $false)
    if ($hMenu) { [Win32.User32]::DeleteMenu($hMenu, 0xF060, 0x00000000) | Out-Null } # Disable Close
})

# [REQ 2] Magnet Windows: Sync Console Position when Menu Moves
$Window.Add_LocationChanged({
    if (!$Silent) {
        $ConsX = ($Window.Left * $Scale) - $ConsoleW_Px - ($Gap) # Calculate target Console X (Pixels)
        $ConsY = ($Window.Top * $Scale)
        # Force Console to follow and stay TopMost
