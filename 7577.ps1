...
# =========================================================
#  YOUTUBE TV INSTALLER v75.7.7 (ELEVATION FIXED)
#  Version: 2.0 Build 23.75.7.7
#  File: 7577.ps1 | Branch: branch
#  Status: Fixed Admin Crash | Safe Argument Passing
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION & LANGUAGE
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.7.7"
...
$LangDict = @{
    "EN" = @{
        "Title"="YouTube TV Installer"; "Dev"="Developed by IT Groceries Shop"; "Refresh"="Installed a new browser? Click Refresh"
        "Rec"="(Recommended)"; "ClickDL"="(Click to Download)"; "Facebook"="Facebook"; "GitHub"="GitHub"; "About"="About"
        "Desktop"="Desktop"; "StartMenu"="StartMenu"; "Uninstall"="Uninstall"; "Start"="Start Install"; "Exit"="EXIT"
        "Processing"="Processing..."; "Finished"="Finished"; "UninsSel"="Uninstall Selected"; "ModeTV"="YouTube TV"; "ModePC"="YouTube PC"
    }
    "TH" = @{
        "Title"="ตัวติดตั้ง YouTube TV"; "Dev"="พัฒนาโดย IT Groceries Shop"; "Refresh"="ติดตั้งเบราว์เซอร์ใหม่? คลิกเพื่อรีเฟรช"
        "Rec"="(แนะนำ)"; "ClickDL"="(คลิกเพื่อดาวน์โหลด)"; "Facebook"="Facebook"; "GitHub"="GitHub"; "About"="เกี่ยวกับ"
        "Desktop"="หน้าจอหลัก"; "StartMenu"="เมนูเริ่ม"; "Uninstall"="ถอนการติดตั้ง"; "Start"="เริ่มติดตั้ง"; "Exit"="ออก"
        "Processing"="กำลังทำงาน..."; "Finished"="เสร็จสิ้น"; "UninsSel"="ยืนยันการลบ"; "ModeTV"="โหมดทีวี (TV)"; "ModePC"="โหมดคอม (PC)"
    }
}
...
# ---------------------------------------------------------
# [4] ADMIN ELEVATION (RE-WRITTEN FOR STABILITY)
# ---------------------------------------------------------
if (-not $Silent -and -not $IsAdmin) {
    # 1. Prepare Arguments String (Safe Passing)
    $PassArgs = @()
    if ($Browser -ne "Ask") { $PassArgs += "-Browser `"$Browser`"" } # Quote string values
    ...
    try {
        if ($TargetFile -and (Test-Path $TargetFile)) {
            # Case A: Local File (.cmd or .ps1)
            if ($TargetFile -match '\.(cmd|bat)$') {
                # Run .cmd directly (OS handles it)
                Start-Process -FilePath "$TargetFile" -ArgumentList "$ArgStr" -Verb RunAs -WindowStyle Hidden
            } else {
                # Run .ps1 via PowerShell
                Start-Process -FilePath "powershell" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$TargetFile`" $ArgStr" -Verb RunAs
            }
        } else {
            # Case B: Memory/Web (Download first)
            (New-Object System.Net.WebClient).DownloadFile($SelfURL, $TempScript)
            Start-Process -FilePath "powershell" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$TempScript`" $ArgStr" -Verb RunAs
        }
    } catch { 
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.7.7 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
...
"@
    ...
    function Update-Language {
        ...
        $BLang.Content = "[ $script:CurrentLang ]"
        Load-BrowserList
    }
