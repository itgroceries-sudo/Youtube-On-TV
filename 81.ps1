...
# =========================================================
#  YOUTUBE TV INSTALLER v81.0 (FINAL ALIGNMENT FIX)
#  Status: Center UI | Magnet Fixed | Taskbar Ghost Stable
# =========================================================
...
$SelfURL = "$GitHubRaw/81.ps1"
$AppVersion = "2.0 Build 27.81"
$BuildDate  = "30-1-2026"
...
# [FIX] Better Initial Alignment (Center Menu, Console on Left)
$MenuW_Px = [int]($BaseW * $Scale)
$TotalW_Px = $ConsoleW_Px + $MenuW_Px + [int]($Gap * $Scale)
$Start_X_Px = ($Scr.Width - $TotalW_Px) / 2
$Start_Y_Px = ($Scr.Height - $ConsoleH_Px) / 2
...
if ($Silent) {
    ...
    # Init Console Position
    [Win32.User32]::SetWindowPos($ConsoleHandle, [IntPtr]-1, [int]$Start_X_Px, [int]$Start_Y_Px, [int]$ConsoleW_Px, [int]$ConsoleH_Px, 0x0040) | Out-Null
...
# GUI XAML (v81 - ALIGNMENT STABILITY)
[xml]$xaml = @"
...
# Set Initial Menu Location (Right of Console)
$Window.Left = ($Start_X_Px + $ConsoleW_Px + [int]($Gap * $Scale)) / $Scale
$Window.Top  = $Start_Y_Px / $Scale
