...
# =========================================================
#  YOUTUBE TV INSTALLER v79.0 (BLUE BORDER EDITION)
#  Status: Blue Window Border | Stable Magnet | Ghost Console
# =========================================================
...
$SelfURL = "$GitHubRaw/79.ps1"
$AppVersion = "2.0 Build 25.79"
$BuildDate  = "30-1-2026"
...
$BaseW = 500; $BaseH = 820; $Gap = 2 # Small gap for border visibility
...
Title="YouTube TV Installer" Height="$BaseH" Width="$BaseW" WindowStartupLocation="Manual" ResizeMode="NoResize" Background="#2196F3" Topmost="True" AllowsTransparency="True" WindowStyle="None">
    <Window.Resources>
    ...
    <Border BorderBrush="#2196F3" BorderThickness="2" Background="#181818" MouseLeftButtonDown="Window_Drag">
        <Grid Margin="25">
...
$Window.Add_MouseLeftButtonDown({ $this.DragMove() }) # WindowStyle=None needs manual drag
...
foreach ($b in $Global:Browsers) {
    ...
    $Bor.Add_MouseLeftButtonUp({ param($s, $e)
        $cb = $s.Child.Children[2]
        if ($cb.IsEnabled) { $cb.IsChecked = -not $cb.IsChecked } else { if ($s.Tag) { Start-Process $s.Tag } }
    })
}
