...
# =========================================================
#  YOUTUBE TV INSTALLER v75.6.4 (REFRESH FIX)
#  Version: 2.0 Build 23.75.6.4
#  File: 7564.ps1 | Branch: branch
#  Status: Refresh Button Centered & Styled
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.6.4"
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.6.4 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
...
function Install-Browser {
    ...
    $LnkName = "YouTube On TV - $($Obj.N).lnk"; $Targets = @($Desktop); if ($UseStartMenu) { $Targets += $StartMenu }
    if ($Uninstall) { foreach ($p in $Targets) { $FullPath = Join-Path $p $LnkName; if (Test-Path $FullPath) { Remove-Item $FullPath -Force; if(!$Silent){ Write-Host " [REMOVE] $FullPath" -ForegroundColor Red } } } return }
    if (!$Obj.Path) { return }
...
function Load-BrowserList {
    ...
    # [FIXED] Refresh Button centered and looking like a button
    ...
    $RefRow.Children.Add($RefView)|Out-Null; $RefRow.Children.Add($RefTxt)|Out-Null
...
$BA.Add_Click({ Play-Sound "Click"; $Sel = $Stack.Children | Where-Object { $_.Child.Children[2].IsChecked }; if ($Sel.Count -eq 0) { return }; $BA.IsEnabled = $false; $BA.Content = "Processing..."; $IsUninstall = ($BTrash.Tag -eq "On"); $UseStartMenu = ($BWin.Tag -eq "On"); foreach ($i in $Sel) { Install-Browser $i.Child.Children[2].Tag $IsUninstall $UseStartMenu }; $BA.Content = "Finished"; Play-Sound "Done"; Start-Sleep 2; $BA.IsEnabled = $true; if ($IsUninstall) { $BA.Content = "Uninstall Selected" } else { $BA.Content = "Start Install" } })
