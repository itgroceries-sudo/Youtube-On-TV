...
# =========================================================
#  YOUTUBE TV INSTALLER v75.9.6 (CLASSIC CONSOLE)
#  Version: 2.0 Build 23.75.9.6
#  File: 7596.ps1 | Branch: branch
#  Status: Black Console | Native TitleBar | Fitted Launcher
# =========================================================

# ---------------------------------------------------------
# [1] CONFIG & LANGUAGE
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.9.6"
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    # [FIX] Resize Launcher to fit text
    try { $host.UI.RawUI.WindowSize = New-Object System.Management.Automation.Host.Size(85, 22) } catch {}
...
if ($Silent) { if ($ConsoleHandle) { [Win32.User32]::ShowWindow($ConsoleHandle, 0) | Out-Null } } else {
    if ($ConsoleHandle) { 
        ...
        # [FIX] NO Style modification here (Keep Native Frame)
...
foreach($k in $Assets.Keys){ $D="$InstallDir\$k.ico"; if(!(Test-Path $D) -or (Get-Item $D).Length -lt 100){ try{ (New-Object Net.WebClient).DownloadFile($Assets[$k],$D) }catch{} } }

# [FIX] Apply Icon & Black Background
$ConsoleIcon = "$InstallDir\ConsoleIcon.ico"
if (!$Silent) {
    $host.UI.RawUI.WindowTitle = "Installer Log Console"
    $host.UI.RawUI.BackgroundColor = "Black" # [FIX] Black Background
    ...
    Write-Host "==========================================" -ForegroundColor Yellow
    Write-Host " [ICON]     Set Console Icon." -ForegroundColor DarkGray
}

$LocalIcon = "$InstallDir\MenuIcon.ico"
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.9.6 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
