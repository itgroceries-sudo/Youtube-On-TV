...
# =========================================================
#  YOUTUBE TV INSTALLER v76.0.2 (PERFECT SYNC)
#  Version: 2.0 Build 23.76.0.2
#  File: 7602.ps1 | Branch: main
#  Status: Console Forced to Match GUI Pixel-Perfect
# =========================================================
...
# [1] CONFIG & LANGUAGE
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.76.0.2"
...
# [TUNING] GUI Size
$GuiW = 600
$GuiH = 860 # Increased Height for Browsers

$Scr = [System.Windows.Forms.Screen]::PrimaryScreen.WorkingArea
$GuiW_Px = [int]($GuiW * $Scale); $GuiH_Px = [int]($GuiH * $Scale)

# Calculate Start Position (Centered Side-by-Side)
$TotalWidth_Px = $GuiW_Px * 2
$StartX_Px = ($Scr.Width - $TotalWidth_Px) / 2
$StartY_Px = ($Scr.Height - $GuiH_Px) / 2

# WPF Coords
$WindowX_WPF = ($StartX_Px + $GuiW_Px) / $Scale
$WindowY_WPF = $StartY_Px / $Scale

if ($Silent) { if ($ConsoleHandle) { [Win32.User32]::ShowWindow($ConsoleHandle, 0) | Out-Null } } else {
    # EXPAND BUFFER to ensure Console can be stretched
    ...
    if ($ConsoleHandle) { 
        ...
        # [KEY FIX] Force Console to Match GUI Pixel Height Exactly
        # Console goes to LEFT ($StartX_Px)
        # Size matches GUI ($GuiW_Px, $GuiH_Px)
        [Win32.User32]::SetWindowPos($ConsoleHandle, [IntPtr]0, [int]$StartX_Px, [int]$StartY_Px, [int]$GuiW_Px, [int]$GuiH_Px, 0x0040) | Out-Null
    }
}
...
if (!$Silent) {
    ...
    if (Test-Path $ConsoleIcon) { $h=[Win32.User32]::LoadImage([IntPtr]::Zero, $ConsoleIcon, 1, 0, 0, 0x10); if($h){ [Win32.User32]::SendMessage($ConsoleHandle, 0x80, [IntPtr]0, $h) | Out-Null; [Win32.User32]::SendMessage($ConsoleHandle, 0x80, [IntPtr]1, $h) | Out-Null } }
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.76.0.2 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
...
$Global:Browsers = @(
    @{N="Google Chrome"; E="chrome.exe"; K="Chrome"; URL="https://www.google.com/chrome/"; P=@("$PF\Google\Chrome\Application\chrome.exe","$PF86\Google\Chrome\Application\chrome.exe"); IsFound=$false; ActualPath=$null}
    @{N="Microsoft Edge"; E="msedge.exe"; K="Edge"; URL="https://www.microsoft.com/edge"; P=@("$PF86\Microsoft\Edge\Application\msedge.exe","$PF\Microsoft\Edge\Application\msedge.exe"); IsFound=$false; ActualPath=$null}
...
function Scan-Browsers {
    foreach ($b in $Global:Browsers) {
        foreach ($p in $b.P) { if ($p -and (Test-Path $p)) { $b.IsFound = $true; $b.ActualPath = $p; break } }
    }
}
...
function Install-Browser {
    ...
    $ArgList = "/c $KillCmd start `"`" `"$($Obj.ActualPath)`" --profile-directory=Default --app=$AppUrl $UAStr --start-fullscreen --disable-features=CalculateNativeWinOcclusion --disable-renderer-backgrounding --disable-background-timer-throttling"
...
if ($Silent -or ($Browser -ne "Ask")) { Scan-Browsers; foreach($b in $Global:Browsers){ if($b.N -match $Browser -or $b.K -match $Browser){ if($b.IsFound){ Install-Browser $b.N $false $AddStartMenu $true $false } } }; exit }
...
Title="YouTube TV Installer" Height="$GuiH" Width="$GuiW" WindowStartupLocation="Manual" ResizeMode="NoResize" Background="#181818" WindowStyle="None" BorderBrush="#2196F3" BorderThickness="2">
    <Window.Resources>
...
"@
    ...
    function Render-BrowserList {
        ...
        foreach ($b in $Global:Browsers) {
            ...
            if ($b.IsFound) { } 
            else { $Txt.Text += " $($D['ClickDL'])"; $Txt.Foreground="#55AAFF"; $Chk.IsEnabled=$false; $Chk.IsChecked=$false; $Bor.Opacity=0.8; $Bor.Tag = $b.URL }
            ...
            $Bor.Add_MouseLeftButtonUp({ param($sender, $e); Play-Sound "Tick"; $cb = $sender.Child.Children[2]; if ($cb.IsEnabled) { $cb.IsChecked = -not $cb.IsChecked } else { if ($sender.Tag -ne $null -and $sender.Tag.StartsWith("http")) { Start-Process $sender.Tag } } })
        }
        ...
        $RefBor.Add_MouseLeftButtonUp({ Play-Sound "Click"; Scan-Browsers; Render-BrowserList })
    }
    ...
    Scan-Browsers; Update-Language; Update-StartButton
