...
# =========================================================
#  YOUTUBE TV INSTALLER v75.9.7 (ASSET & LAYOUT FIX)
#  Version: 2.0 Build 23.75.9.7
#  File: 7597.ps1 | Branch: branch
#  Status: TLS1.2 Assets | Fitted Launcher | Synced Height
# =========================================================

# ---------------------------------------------------------
# [1] CONFIG & LANGUAGE
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.9.7"
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    # [FIX] Force Resize again for certainty
...
if ($Silent) { if ($ConsoleHandle) { [Win32.User32]::ShowWindow($ConsoleHandle, 0) | Out-Null } } else {
    if ($ConsoleHandle) { 
        ...
        # Position Console LEFT, Sync Height
...
foreach($k in $Assets.Keys){ 
    ...
    if(!(Test-Path $D) -or (Get-Item $D).Length -lt 100){ 
        try{ 
            ...
            if(!$Silent){ Write-Host " [DOWNLOAD] OK: $k" -ForegroundColor Green } # [FIX] Restore Log
        }catch{
            if(!$Silent){ Write-Host " [ERROR]    Fail: $k" -ForegroundColor Red }
        } 
    } else { 
        if(!$Silent){ Write-Host " [CACHE]    OK: $k" -ForegroundColor DarkGray } # [FIX] Restore Log
    } 
}

$LocalIcon = "$InstallDir\MenuIcon.ico"; $ConsoleIcon = "$InstallDir\ConsoleIcon.ico"
# Reload Console Icon in case it was just downloaded
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.9.7 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
