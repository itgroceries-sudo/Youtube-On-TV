...
# =========================================================
#  YOUTUBE TV INSTALLER v75.3 (SILENT FIXED)
#  Based on: v75.2 + v90 Logic
#  Status: Silent Works | Internal Console | Blue Frame
# =========================================================
...
$SelfURL = "$GitHubRaw/75.3.ps1"
$AppVersion = "2.0 Build 23.75.3"
...
# --- [3. ADMIN CHECK (Pass Args Correctly)] ---
...
if ($Silent) {
    ...
    # Logic: If specific browser requested, try that. Else default to Brave.
...
foreach ($b in $Global:Browsers) {
    ...
    if ($FP) { 
        ...
        $Bor.Tag = $b.URL # Swap Tag to URL for download
    }
...
$BA.Add_Click({
    ...
    $BA.IsEnabled = $false; $BA.Content = "Processing..."; foreach ($i in $Sel) { Install-BrowserLogic $i.Child.Children[2].Tag }; 
