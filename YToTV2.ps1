...
# =========================================================
#  YOUTUBE TV INSTALLER v74.0 (HYBRID FIXED)
#  Status: Offline Hybrid Support | Admin Pause | Bye Delay
# =========================================================
...
# Check if running locally (Hybrid/Downloaded) or from Web (IEX)
$IsLocalFile = ($PSScriptRoot -or $ScriptPath)
if ($ScriptPath) { $TargetFile = $ScriptPath } # Hybrid .cmd
elseif ($PSScriptRoot) { $TargetFile = $PSCommandPath } # Normal .ps1

# --- [4. WEB LAUNCH CHECK (Only if NOT local)] ---
if (-not $IsLocalFile) {
    if (!$Silent) { 
    ...
    $TargetFile = "$env:TEMP\YToTV.ps1"
    try { (New-Object System.Net.WebClient).DownloadFile($SelfURL, $TargetFile) } catch { exit }
    
    # Relaunch the downloaded .ps1
    $PassArgs = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "`"$TargetFile`"")
...
# --- [5. CONSOLE PRE-LOAD (To show Admin Msg)] ---
# We need console visible NOW to show Admin Request message
...
if (-not $Silent -and -not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Read-Host # [REQ 1] Pause for user
    
    # [REQ 2 & FIX HYBRID] Relaunch Logic
    $PassArgs = @()
    if ($Browser -ne "Ask") { $PassArgs += "-Browser"; $PassArgs += $Browser }
    
    if ($TargetFile -match "\.cmd$" -or $TargetFile -match "\.bat$") {
        # Hybrid Mode: Run the .cmd file directly (Do NOT use PowerShell -File)
        Start-Process "$TargetFile" -ArgumentList $PassArgs -Verb RunAs
    } else {
        # PS1 Mode: Run via PowerShell
        $PSArgs = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "`"$TargetFile`"") + $PassArgs
...
# --- [8. CONSOLE SETUP (Retro)] ---
$ConsoleHandle = [Win32.Win32]::GetConsoleWindow()
# Redefine Utils for Window positioning
...
$BC.Add_Click({ 
    # [REQ 2] Clean & Bye Message with Delay
