...
# =========================================================
#  YOUTUBE TV INSTALLER (CORE v46.0)
#  Engine: VMD-Based Win32 API (Force Show Console)
# =========================================================
...
# --- [VMD STYLE] WIN32 API ---
# ใช้ SetWindowPos เป็นพระเอก (เหมือน VMD) เพื่อจัดการทั้งตำแหน่งและการโชว์หน้าต่าง
...
# --- CONSOLE VISIBILITY (VMD LOGIC) ---
$ConsoleHandle = $Win32::GetConsoleWindow()

if ($Silent) {
    # Silent: Hide completely
    $Win32::ShowWindow($ConsoleHandle, 0) | Out-Null
} else {
    # Normal: Setup Console Appearance
    ...
    # [THE VMD MAGIC]
    # 0x0040 = SWP_SHOWWINDOW (บังคับโชว์หน้าต่างทันที พร้อมย้ายตำแหน่ง)
    # วิธีนี้จะ Override สถานะ Hidden ที่มาจาก Header ได้แน่นอน
    $Win32::SetWindowPos($ConsoleHandle, [IntPtr]::Zero, [int]$X, [int]$Y, [int]$W, [int]$H, 0x0040) | Out-Null
}
...
if(Test-Path $ConsoleIcon){ 
    ...
    if($h){ [Native.Win32]::SendMessage($ConsoleHandle,0x80,0,$h)|Out-Null; [Native.Win32]::SendMessage($ConsoleHandle,0x80,1,$h)|Out-Null } 
}

if(!$Silent){ Write-Host "`n    YOUTUBE TV INSTALLER v46.0" -ForegroundColor Cyan; Write-Host "[INIT] Ready..." }
...
# XAML UI (With GitHub Vector Icon Restored)
[xml]$xaml = @"
    ...
    <Grid Margin="25"><Grid.RowDefinitions><RowDefinition Height="Auto"/><RowDefinition Height="20"/><RowDefinition Height="*"/><RowDefinition Height="Auto"/></Grid.RowDefinitions>
        ...
        <Grid Grid.Row="3" Margin="0,20,0,0"><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>
            <StackPanel Orientation="Horizontal" Grid.Column="0">
                 ...
                 <Button x:Name="BG" Width="45" Height="45" Background="#333333" Style="{StaticResource Bn}" ToolTip="GitHub" Cursor="Hand">
...
# Align GUI next to Console
try {
    $Win.Left = [int]$X + [int]$W + [int]$Gap
    $Win.Top  = [int]$Y
} catch {}
