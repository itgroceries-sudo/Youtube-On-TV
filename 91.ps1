...
# =========================================================
#  YOUTUBE TV INSTALLER v91.0 (PATH DETECTION FIX)
#  Status: Path Check Restored | Silent & Stealth Ready
# =========================================================
...
$SelfURL = "$GitHubRaw/91.ps1"
$AppVersion = "2.0 Build 32.91"
...
# --- [4. BROWSER DATABASE & LOGIC] ---
# [FIX] Define paths explicitly to ensure 64/32 bit detection works correctly
$PF = $env:ProgramFiles
$PF86 = ${env:ProgramFiles(x86)}
if (!$PF86) { $PF86 = "C:\Program Files (x86)" } # Fallback
$L = $env:LOCALAPPDATA
...
function Install-Logic($BrowserName) {
    $Obj = $Global:Browsers | Where-Object { $_.N -match $BrowserName -or $_.K -match $BrowserName }
    if ($Obj) {
...
if ($IsSilent) {
    ...
    $TargetBrowser = if ($Browser -eq "Ask") { "Brave" } else { $Browser }
    Install-Logic $TargetBrowser | Out-Null
...
Title="YouTube TV Installer" Height="880" Width="520" WindowStartupLocation="CenterScreen" ResizeMode="NoResize" Background="#181818" Topmost="True" WindowStyle="None" BorderBrush="#2196F3" BorderThickness="2">
    ...
    <Grid Margin="25,20,25,25">
        ...
        <Border Grid.Row="3" Background="#000000" CornerRadius="8" Margin="0,15,0,0" BorderBrush="#333333" BorderThickness="1"><ScrollViewer x:Name="LogScroll" VerticalScrollBarVisibility="Auto"><TextBlock x:Name="LogBox" Foreground="#00FF00" FontFamily="Consolas" FontSize="11" Margin="10" TextWrapping="Wrap" Text="[SYSTEM] Initializing UI..."/></ScrollViewer></Border>
