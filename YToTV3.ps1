...
# =========================================================
#  YOUTUBE TV INSTALLER v74.0 (FINAL FIX)
#  Status: Admin Msg First | Offline Fixed | Cleanup Logic
# =========================================================

# --- [1. INITIAL SETUP] ---
$InstallDir = "$env:TEMP\ITG_YToTV"
$TempScript = "$env:TEMP\YToTV3.ps1"
...
# We need this visible NOW for the Admin Message
...
# --- [3. ADMIN CHECK (GLOBAL)] ---
# This now runs BEFORE any download logic
...
if (-not $Silent -and -not $Principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    ...
    Read-Host # [REQ 1 & 2] Pause for user (Both Offline & IEX)
    ...
    if (!$IsLocal) {
        # [IEX CASE] Must download first, then RunAs
    ...
    elseif ($TargetFile -match "\.cmd$" -or $TargetFile -match "\.bat$") {
        # [OFFLINE CMD CASE] Fix for "Vanishing" - Use cmd /c
    ...
    exit # Close un-elevated process
}

# --- [4. IEX DOWNLOAD HANDLER (If Admin but IEX)] ---
# If we are IEX but somehow Admin already (or after elevation), ensure we have a file path? 
# Actually, if we are IEX, we are running in memory. 
# But the asset logic needs a place. The script continues in memory fine.
