...
# =========================================================
#  YOUTUBE TV INSTALLER (ULTIMATE EDITION)
#  Version: 2.0 Build 23.75.5.8
#  Status: Clean Code | Fixed Colors | Crash Proof | Full Option
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION & ARGUMENTS
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.5.8"
...
$SelfURL    = "$GitHubRaw/7558.ps1"

# Resolve Script Path (Vital for Admin Elevation)
...
if ($Silent) {
    ...
    # Show Console (5) BUT Remove Close Button logic
...
# ---------------------------------------------------------
# [3] ADMIN PRIVILEGES CHECK (CRASH PROOF)
# ---------------------------------------------------------
...
if(!$Silent){ 
    ...
    Write-Host "   (V.2 Build 23.75.5.8 : $BuildDate)     " -ForegroundColor Green
...
function Install-Browser {
    ...
    $PathsToCheck = @( (Join-Path $Desktop $LnkName) )
    if ($UseStartMenu) { $PathsToCheck += (Join-Path $StartMenu $LnkName) }

    if ($Uninstall) {
        foreach ($p in $PathsToCheck) {
            if (Test-Path $p) { Remove-Item $p -Force; if(!$Silent){ Write-Host " [REMOVE] $p" -ForegroundColor Red } }
        }
        return
    }
    ...
    foreach ($TargetDir in @($Desktop, if($UseStartMenu){$StartMenu})) {
    ...
    <Grid Margin="25">
        ...
        <Grid Grid.Row="4" Margin="0,20,0,0"><Grid.ColumnDefinitions><ColumnDefinition Width="Auto"/><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>
            ...
            <StackPanel Orientation="Horizontal" Grid.Column="2" HorizontalAlignment="Right">
                ...
                <Button x:Name="BA" Content="Start Install" Width="140" Height="45" Background="#2E7D32" Foreground="White" Style="{StaticResource Btn}" Cursor="Hand"/>
            </StackPanel>
        </Grid>
    </Grid>
</Window>
