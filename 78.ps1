...
# =========================================================
#  YOUTUBE TV INSTALLER v78.0 (GHOST CONSOLE)
#  Status: Console Hidden from Taskbar | State Synced
# =========================================================
...
$SelfURL = "$GitHubRaw/78.ps1"
$AppVersion = "2.0 Build 24.78"
$BuildDate  = "30-1-2026"
...
if ($Silent) {
    ...
    # [REQ v77] REMOVE 'X' BUTTON (Remove WS_SYSMENU)
    $GWL_STYLE = -16
    ...
    # [REQ v78] HIDE FROM TASKBAR (Add WS_EX_TOOLWINDOW)
    $GWL_EXSTYLE = -20
    $WS_EX_TOOLWINDOW = 0x00000080
    $CurrentExStyle = [Win32.User32]::GetWindowLong($ConsoleHandle, $GWL_EXSTYLE)
    [Win32.User32]::SetWindowLong($ConsoleHandle, $GWL_EXSTYLE, ($CurrentExStyle -bor $WS_EX_TOOLWINDOW)) | Out-Null
...
if(!$Silent){ 
    ...
    Write-Host "   (V.2 Build 24.78 : $BuildDate)         " -ForegroundColor Green
...
function Install-Browser {
    ...
    $s.Save()
    if(!$Silent){ Write-Host " [INSTALL] $($Obj.N)... " -NoNewline -ForegroundColor White; Write-Host "DONE" -ForegroundColor Green }
}
...
# GUI Init Logic (Magnet + NoClose + StateSync)
$Window.Add_SourceInitialized({
    $hwnd = (New-Object System.Windows.Interop.WindowInteropHelper($Window)).Handle
    # Remove Close Button from WPF
    $GWL_STYLE = -16; $WS_SYSMENU = 0x00080000
    $CurStyle = [Win32.User32]::GetWindowLong($hwnd, $GWL_STYLE)
    [Win32.User32]::SetWindowLong($hwnd, $GWL_STYLE, ($CurStyle -band (-not $WS_SYSMENU))) | Out-Null
})

$Window.Add_LocationChanged({
    if (!$Silent) {
        $ConsX = ($Window.Left * $Scale) - $ConsoleW_Px - ($Gap)
...
# [REQ v78] Sync Console State with WPF Window (Minimize together)
$Window.Add_StateChanged({
    if (!$Silent) {
        if ($Window.WindowState -eq "Minimized") {
            ...
            # Show Console (No Activate to keep focus on GUI)
            [Win32.User32]::ShowWindow($ConsoleHandle, 4) | Out-Null # 4 = SW_SHOWNOACTIVATE
        }
    }
})
...
foreach ($b in $DetectedList) {
    ...
    $Img = New-Object System.Windows.Controls.Image; $Img.Width = 32; $Img.Height = 32; if (Test-Path $b.Img) { $Img.Source = $b.Img }
    ...
    if(!$b.Inst){ $Txt.Text += " (Click to Download)"; $Txt.Foreground="#55AAFF"; $Chk.IsEnabled=$false; $Chk.IsChecked=$false; $Bor.Opacity=0.8 }
    ...
    $Bor.Add_MouseLeftButtonUp({ param($sender, $e)
