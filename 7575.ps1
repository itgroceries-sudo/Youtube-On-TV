...
# =========================================================
#  YOUTUBE TV INSTALLER v75.7.5 (ANTI-CRASH)
#  Version: 2.0 Build 23.75.7.5
#  File: 7575.ps1 | Branch: branch
#  Status: Elevation Fix | Crash Guard | Full Features
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION & GLOBAL VARS
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.7.5"
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    try {
        if ($TargetFile -and (Test-Path $TargetFile)) {
            # [FIX] Check Extension: If CMD/BAT, run directly. If PS1, run via PowerShell.
            if ($TargetFile -match '\.(cmd|bat)$') {
                Start-Process "$TargetFile" -ArgumentList $PassArgs -Verb RunAs
            } else {
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.7.5 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
...
# ---------------------------------------------------------
# [7] XAML UI & [8] EVENTS (Wrapped in Crash Guard)
# ---------------------------------------------------------
try {
...
"@
    ...
    # [CRASH GUARD] Show error if GUI fails
