...
# =========================================================
#  YOUTUBE TV INSTALLER v75.7.4 (INTERNATIONAL)
#  Version: 2.0 Build 23.75.7.4
#  File: 7574.ps1 | Branch: branch
#  Status: Auto-Detect Region | TH/EN Toggle | Real-time Swap
# =========================================================

# ---------------------------------------------------------
# [1] CONFIGURATION & LANGUAGE DICT
# ---------------------------------------------------------
$AppVersion = "2.0 Build 23.75.7.4"
$BuildDate  = "05-02-2026"
...
$LangDict = @{
    "EN" = @{
        "Title"      = "YouTube TV Installer"
        "Dev"        = "Developed by IT Groceries Shop"
        "Refresh"    = "Installed a new browser? Click Refresh"
        "Rec"        = "(Recommended)"
        "ClickDL"    = "(Click to Download)"
        "Facebook"   = "Facebook"
        "GitHub"     = "GitHub"
        "About"      = "About"
        "Desktop"    = "Desktop"
        ...
        "Finished"   = "Finished"
        "UninsSel"   = "Uninstall Selected"
        "ModeTV"     = "YouTube TV"
        "ModePC"     = "YouTube PC"
    }
    "TH" = @{
        "Title"      = "ตัวติดตั้ง YouTube TV"
        "Dev"        = "พัฒนาโดย IT Groceries Shop"
        "Refresh"    = "ติดตั้งเบราว์เซอร์ใหม่? คลิกเพื่อรีเฟรช"
        "Rec"        = "(แนะนำ)"
        "ClickDL"    = "(คลิกเพื่อดาวน์โหลด)"
        "Facebook"   = "Facebook"
        "GitHub"     = "GitHub"
        "About"      = "เกี่ยวกับ"
        "Desktop"    = "หน้าจอหลัก"
...
$CurrentLang = if ($SysRegion -eq "th") { "TH" } else { "EN" }
...
if (-not $Silent -and -not $IsAdmin) {
    ...
    try {
        if ($TargetFile -and (Test-Path $TargetFile)) { Start-Process "powershell" -ArgumentList (@("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "`"$TargetFile`"") + $PassArgs) -Verb RunAs } 
        else { (New-Object System.Net.WebClient).DownloadFile($SelfURL, $TempScript); Start-Process "powershell" -ArgumentList (@("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "`"$TempScript`"") + $PassArgs) -Verb RunAs }
...
if ($Silent) { [Win32.User32]::ShowWindow($ConsoleHandle, 0) | Out-Null } else {
    ...
    if (($CurrentStyle -band $WS_SYSMENU) -eq $WS_SYSMENU) { [Win32.User32]::SetWindowLong($ConsoleHandle, $GWL_STYLE, ($CurrentStyle -band (-not $WS_SYSMENU))) | Out-Null; [Win32.User32]::SetWindowPos($ConsoleHandle, [IntPtr]0, 0, 0, 0, 0, 0x0027) | Out-Null }
}
...
if(!$Silent){ Write-Host "`n==========================================" -ForegroundColor Green; Write-Host "   (V.2 Build 23.75.7.4 : $BuildDate)     " -ForegroundColor Green; Write-Host "==========================================" -ForegroundColor Green; Write-Host " [INIT] Scanning installed browsers..." -ForegroundColor Green }
    ...
    <Grid Margin="25">
        ...
        <Grid Grid.Row="0">
            ...
            <StackPanel Grid.Column="1" VerticalAlignment="Center" Margin="20,0,0,0">
                <TextBlock x:Name="T_Title" Text="YouTube TV Installer" Foreground="White" FontSize="28" FontWeight="Bold"><TextBlock.Effect><DropShadowEffect Color="#FF0000" BlurRadius="15" Opacity="0.6"/></TextBlock.Effect></TextBlock>
                <StackPanel Orientation="Horizontal" Margin="2,5,0,0"><TextBlock x:Name="T_Dev" Text="Developed by IT Groceries Shop &#x2665;" Foreground="#FF0000" FontSize="14" FontWeight="Bold"/></StackPanel>
            </StackPanel>
            <StackPanel Grid.Column="2" Orientation="Horizontal" HorizontalAlignment="Right" VerticalAlignment="Top" Margin="0,10,0,0">
                <Button x:Name="BLang" Content="[ EN ]" Width="70" Height="40" Background="#444" Foreground="White" Style="{StaticResource Btn}" Margin="0,0,10,0" Cursor="Hand" FontWeight="Bold"/>
                <ComboBox x:Name="ComboMode" Width="150" Height="40" Foreground="White" FontSize="16" FontWeight="Bold" Cursor="Hand"><ComboBoxItem Content="YouTube TV" IsSelected="True"/><ComboBoxItem Content="YouTube PC"/></ComboBox>
            </StackPanel>
        </Grid>
        ...
        <Grid Grid.Row="3" Margin="0,15,0,8">
            ...
            <StackPanel Orientation="Horizontal" Grid.Column="2" HorizontalAlignment="Right">
                <Button x:Name="BA" Content="Start Install" Width="150" Height="50" Background="#2E7D32" Foreground="White" Style="{StaticResource Btn}" Margin="0,0,0,0" Cursor="Hand" FontSize="16"/>
            </StackPanel>
        </Grid>
...
function Update-Language {
    $D = $LangDict[$CurrentLang]
    $Window.FindName("T_Title").Text = $D["Title"]
    $Window.FindName("T_Dev").Text   = $D["Dev"]
    $Window.FindName("T_Desk").Text  = $D["Desktop"]
    $Window.FindName("T_Win").Text   = $D["StartMenu"]
    $Window.FindName("T_Unins").Text = $D["Uninstall"]
    $Window.FindName("T_FB").Text    = $D["Facebook"]
    $Window.FindName("T_Git").Text   = $D["GitHub"]
    $Window.FindName("T_Abt").Text   = $D["About"]
    $BC.Content = $D["Exit"]
    $ComboMode.Items[0].Content = $D["ModeTV"]
    $ComboMode.Items[1].Content = $D["ModePC"]
    
    # Dynamic Button (Start vs Uninstall)
    if ($BTrash.Tag -eq "On") { $BA.Content = $D["UninsSel"] } else { $BA.Content = $D["Start"] }
    $BLang.Content = "[ $CurrentLang ]"
    
    # Reload Browser List (for labels inside)
    Load-BrowserList
}

function Load-BrowserList {
    $Stack.Children.Clear(); $D = $LangDict[$CurrentLang]
...
$BLang.Add_Click({ 
    if ($CurrentLang -eq "EN") { $script:CurrentLang = "TH" } else { $script:CurrentLang = "EN" }
    Play-Sound "Click"; Update-Language
})
...
$BTrash.Add_Click({ 
    ...
    if ($BTrash.Tag -eq "Off") { 
        ...
        $BA.Content = $D["UninsSel"]; $BA.Background = [System.Windows.Media.BrushConverter]::new().ConvertFromString("#D32F2F"); Play-Sound "Warn"
    } else { 
        $BTrash.Tag = "Off"; $BTrash.Background = [System.Windows.Media.BrushConverter]::new().ConvertFromString("#333333")
        $BA.Content = $D["Start"]; $BA.Background = [System.Windows.Media.BrushConverter]::new().ConvertFromString("#2E7D32"); Play-Sound "Click"
    }
    Update-StartButton 
})
...
$BA.Add_Click({ 
    ...
    $BA.IsEnabled = $false; $BA.Content = $LangDict[$CurrentLang]["Processing"]
    $IsUninstall = ($BTrash.Tag -eq "On"); $UseStartMenu = ($BWin.Tag -eq "On"); $UseDesktop = ($BDesk.Tag -eq "On")
    $IsPCMode = ($ComboMode.Text -eq $LangDict[$CurrentLang]["ModePC"]) # Check against localized text
    
    foreach ($i in $Sel) { Install-Browser $i.Child.Children[2].Tag $IsUninstall $UseStartMenu $UseDesktop $IsPCMode }
    
    $BA.Content = $LangDict[$CurrentLang]["Finished"]; Play-Sound "Done"; Start-Sleep 2; $BA.IsEnabled = $true
    if ($IsUninstall) { $BA.Content = $LangDict[$CurrentLang]["UninsSel"] } else { $BA.Content = $LangDict[$CurrentLang]["Start"] } 
    Update-StartButton
})
